...
import Data.List (intersperse, intercalate, sort, break)
...
import Text.LaTeX.Packages.AMSMath (math, to, epsilon)
import Text.LaTeX.Packages.AMSSymb (amssymb, checkmark)
import Text.LaTeX.Packages.Beamer (beamer, frame, frametitle, CoverOption(Invisible, Transparent), setbeamercovered, overprint, alert)
...
import Grammar (Symbol(..), Production(..), Grammar(..), isTerminal, terminals, nonterminals, mapAccumLFliped, equalf, addEOFToGrammar, fst3, snd3, trd3, trd4)
...
explain :: LaTeX -> State View ()
explain msg =
  modify $ \view@View{step = i, expl = xs} ->
             view{expl = xs ++ [ Doc [ (Only,BE,i) ] msg ]}
...
build :: Grammar -> View
build grm =
  execState buildView (startView (addEOFToGrammar grm))
...
buildView = do
  -- show the grammar
  get >>= put . showGrammar
  get >>= put . buildNullables
  get >>= put . buildFirsts
  get >>= put . buildFollows
  buildTableLL1
  --
  -- tick
  -- explain "Fim"
...
showGrammar v@View{step = i, grammar = Doc a (s1:s2:ss) : ps, nts, tableDoc, expl} = v'
  ...
  v' = v{ step = i+5
        ...
        , expl = expl ++
                 [ Doc [(Only,BE,i)]
                   (textbf "Esta é a gramática livre de contexto!" <> newline)
                 , Doc [(Only,B,i+1),(Only,E,i+2)]
                   (textbf "Acertando a gramática" <> newline <>
                    "Símbolo inicial: " <> textcolor (DefColor Red) (texy s2) <> newline <>
                    "Novo símbolo inicial: " <> textcolor (DefColor Red) (texy s1) <> newline
                   )
                 , Doc [(Only,BE,i+2)]
                   ("Nova regra de produção: " <> texy (info s1 ::= [info s2, T "$"]) <> newline)
                 , Doc [(Only,B,i+3),(Only,E,i+4)]
                   (textbf "Tabela de símbolos anuláveis, conjuntos first e conjuntos follow" <> newline)
                 ]
        }
...
buildNullables v@View{step = i, grammar, nts, nullable, expl} = v'
  ...
  (i1, grammar1) =
    mapAccumLFliped grammar i $ \i prod@(Doc _ (_:rhs)) ->
      if any (isTerminal . info) rhs
      then (i+1, addDoc [(Uncover,E,i)] prod)
      else (i, prod)
  expl1 = expl ++
          [ Doc [(Only,B,i),(Only,E,i1)]
            (textbf "Encontrando os símbolos não terminais anuláveis" <> newline)
          , Doc [(Only,BE,i)]
            ("Inicialmente nenhum símbolo não terminal é anulável" <> newline)
          , Doc [(Only,B,i+1),(Only,E,i1)]
            ("Removendo as regras de produção com terminais no lado direito" <> newline)
          ]
  -- rules with empty rhs
  ((i2, nullable2, expl2), grammar2) =
    mapAccumLFliped grammar1 (i1+1, nullable1, expl1) $ \(i, n, e) prod ->
      case prod of
       Doc _ (Doc _ lhs : []) ->
         let Just (n1, y, n2) = lookup2 lhs nterms n
             z = [Doc [(Vis,B,i),(Alert,BE,i),(Shade,BE,i)] True]
             prod' = addDoc [(Shade,BE,i),(Uncover,E,i)] prod
             e' = e ++ [Doc [(Only,BE,i)] ("Lado direito da regra é vazio" <> newline)]
         in ((i+1, n1 ++ [y ++ z] ++ n2, e'), prod')
       _ ->
         ((i, n, e), prod)
       ...
       case prod of
        Doc _ (Doc _ lhs : rhs@(_:_))
          ...
          where
            ...
            e' = e ++ [Doc [(Only,BE,i),(Shade2,BE,i)] ("Lado direito da regra é anulável" <> newline)]
        _ ->
          ((i, n, e), prod)
  ...
  grammar4 =
    flip map grammar3 $ \p@(Doc a x) ->
      case last a of
       ...
       _ -> p
  v' = v{step = i3+2, grammar = grammar4, nullable = nullable3, expl = expl3}
...
buildFirsts v@View{step = i, grammar, nts, nullable, first, expl} = v'
  where
    ...
    ((i1, nts1, first1, expl1), grammar1) =
      mapAccumLFliped grammar (i,nts,first,expl) $ \(i, n, f, e) x@(Doc a p@(Doc _ lhs : rhs)) ->
        case rhs of
         Doc _ t@(T _) : _ ->
...
explainFirst1 lhs (t:rhs) set =
  center $
    ...
    texy t <>
    raw (fromString (printf "} edge [->, bend right=90] node[auto,swap]{FIRST} (%s);%%\n" a)) <>
...
buildFollows v@View{step = i, grammar, nts, nullable, first, follow, expl} = v'
  where
    nterms = map info nts
    nullableSet = map (info . fst) (filter (info . last . snd) (zip nts nullable))
    firstSets = zip (map info nts) (map (map info) first)
    ((i1, follow1, expl1), grammar1) =
      mapAccumLFliped grammar (i,follow,expl) $ \(i, f, e) prod@(Doc _ (lhs : rhs)) ->
        case breakAll isNTerminal (map info rhs) of
          -- no nonterminal symbol on right hand side of rule
          [] ->
            ((i+1, f, e'), prod')
            where
              prod' = addDoc [(Shade,BE,i),(Uncover,E,i)] prod
              e' = e ++ [Doc [(Only,BE,i)] (explainFollows1 (info lhs) (map info rhs)) ]

          -- nonterminals on right hand side
          pieces ->
            mapAccumLFliped pieces (i, f, e) $ \(i, f, e) (rhs1, k, rhs2, rhs3) ->
              mapAccumLFliped rhs2 (i, f, e) $ \(i, f, e) symbol ->
                let i' = i + 1
                    Just (f1, y0, f2) = lookup2 k nterms f
                    
                                                 
              
        let -- empty rhs
            rhsLoop rhs1 [] i f e p =
              ((i+1, f, e), p'')
              where
                p' = addDoc [(Shade,E,i)] p
                p'' | isTerminal (last rhs1) = addDoc [(Uncover,E,i)] p'
                    | otherwise = p'
                e' = e ++ [Doc [(Only,BE,i)] (explainFollows2 (info lhs) (map info rhs)) ]
            -- terminals are ignored
            rhsLoop rhs1 (Doc _ k@(T _) : rest) i f e p =
              rhsLoop (rhs1++[k]) rest i f e p
            -- nonterminals are updated
            rhsLoop rhs1 (Doc _ k@(N _) : rest) i f e p = firstOfSeq rest i y0 p
              where
                Just (f1, y0, f2) = lookup2 k nterms f
                firstOfSeq [] i y p =
                  rhsLoop (rhs1++[k]) rest i (f1 ++ [y] ++ f2) e p
                firstOfSeq ((Doc _ t@(T _)) : _) i y p = rhsLoop (rhs1++[k]) rest i' f' e p
                  where
                    z = [ Doc [(Vis,B,i),(Alert,BE,i)] t ]
                    i' = i + 1
                    f' = f1 ++ [mergeDoc y z] ++ f2
                    -- e' = addExplanation e i (explainFollows3 lhs rhs1 k)
                firstOfSeq ((Doc _ t@(N _)) : ts) i y p
                  | elem t nullableSet = firstOfSeq ts i' y' p
                  | otherwise = rhsLoop (rhs1++[k]) rest i' (f1 ++ [y'] ++ f2) e p
                  where
                    Just z0 = lookup t firstSets
                    z = map (Doc [(Vis,B,i),(Alert,BE,i)]) z0
                    i' = i + 1
                    y' = mergeDoc y z
        in rhsLoop [] rhs i f e (addDoc [(Shade,B,i)] prod)
    ((i2, follow2), grammar2) =
      whileChange (equalf (map (map info) . snd . fst)) ((i1, follow1), grammar1) $ \((i, f), g) ->
        mapAccumLFliped g (i, f) $ \(i, f) prod@(Doc _ (Doc _ lhs : rhs)) ->
          let Just (_, anFlwLhs, _) = lookup2 lhs nterms f
              flwLhs = map info anFlwLhs
              rhsLoop (Doc _ k@(N _) : rest) i f p
                | elem k nullableSet =
                    rhsLoop rest i' f' p
                | otherwise =
                    ((i', f'), addDoc [(Shade,E,i)] p)
                where
                  ...
                  i' = i + 1
              rhsLoop _ i f p =
                ((i+1, f), addDoc [(Shade,E,i)] p)
          in rhsLoop (reverse rhs) i f (addDoc [(Shade,B,i)] prod)
    grammar3 =
      flip map grammar2 $ \p@(Doc a x) ->
        case last a of
         (Uncover,E,_) -> Doc (a ++ [(Uncover,B,i2)]) x
         _ -> p
    v' = v{step = i2, grammar = grammar3, follow = follow2, expl = expl1}
...
explainFollows2 :: (LaTeXC l) => Symbol -> [Symbol] -> l
explainFollows2 lhs rhs =
  center $
    texy (lhs ::= rhs) <> par <>
    "O lado direito da regra não termina com símbolo não terminal" <> par <>
    "A regra não não será usada na próxima etapa"

buildTableLL1 :: State View ()
buildTableLL1 = do
  v@View{step = i, grammar, nts, nullable, first, follow, tableLL1, tableLL1Doc} <- get
  let nterms = map info nts
  let nullableSet = map (info . fst) (filter (info . last . snd) (zip nts nullable))
  let firstSets = zip (map info nts) (map (map info) first)
  let followSets = zip (map info nts) (map (map info) follow)
  let ((i1, table1), grammar1) =
        mapAccumLFliped grammar (i, tableLL1) $ \(i, table) prod@(Doc a (Doc b lhs : rhs)) ->
          let (i', nl, fs, rhs') = firstOfSeq nullableSet firstSets i rhs
          ...
          in ((i''+1, table'), Doc a (Doc b lhs : rhs'))
  put v{step = i1, grammar = grammar1, tableLL1 = table1, tableLL1Doc = [(Vis,B,i)]}
...
firstOfSeq
  :: [Symbol]
     ...
     -> Slide
     -> [Doc Symbol]
     -> (Slide, Bool, [Doc Symbol], [Doc Symbol])
firstOfSeq nullableSet firstSets slideNo symbolSeq =
  loopSymbolSeq symbolSeq slideNo [] []
  where
    loopSymbolSeq [] i set previousSyms =
      (i, True, set, reverse previousSyms)
    loopSymbolSeq (sym@(Doc _ s@(T _)) : restSyms) i set previousSyms =
      (i+1, False, set', syms')
      where
        set' = mergeDoc set [Doc [(Vis,B,i),(Alert,BE,i)] s]
        syms' = reverse previousSyms ++ addDoc [(Alert,BE,i)] sym : restSyms
    loopSymbolSeq (sym@(Doc _ s@(N _)) : restSyms) i set previousSyms
      | elem s nullableSet =
          loopSymbolSeq restSyms (i+1) set' previousSyms'
      | otherwise =
          (i+1, False, set', reverse previousSyms' ++ restSyms)
      where
        Just fs = lookup s firstSets
        set' = mergeDoc set (map (Doc [(Vis,B,i),(Alert,BE,i)]) fs)
