...
import Data.List (intersperse, intercalate, sort, inits, tails)
...
buildTableLL1 v@View{step = i, grammar, nts, nullable, first, follow, tableLL1, tableLL1Doc} = v'
  where
    nterms = map info nts
    nullableSet = map (info . fst) (filter (info . last . snd) (zip nts nullable))
    firstSets = zip (map info nts) (map (map info) first)
    ...
    v' = v{step = i1, grammar = grammar1, tableLL1 = table1, tableLL1Doc = [(Vis,B,i)]}
