    ...
    ((i1, table1, expl1), grammar1) =
      mapAccumLFliped grammar (i+1, tableLL1, expl0) $ \(i, table, e) prod@(Doc a (Doc b lhs : rhs)) ->
        let (i', nl, fs, rhs', e') = firstOfSeq i nullableSet firstSets e lhs rhs
            ...
            e'' = addExplanation e' i' (explainLL1 lhs (map info rhs) (if nl then Nothing else Just []) flws [map info fs])
            --e'' = e'
...
explainLL1 lhs rhs1 m_rhs2 flw_lhs fst_rhs1 =
  center $
    tabular Nothing (replicate (2+n1+n2) CenterColumn) (sepBy lnbk [r1,r2]) <>
    par <>
    "NEXT:" <> quad <>
  ...
  where
    ...
    -- r3 = foldl1 (&) (texy flw_lhs : mempty : map texy fst_rhs1 ++ replicate n2 mempty)
